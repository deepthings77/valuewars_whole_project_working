...
const HowItWorks = () => {
  const steps = [
    ...
    },
    {
      ...
      description:
        "After winning an item, the highest bidder will receive an email with the Auctioneer's payment method information, including bank transfer, Stripe, and PayPal.",
    },
    {
      icon: <FaDollarSign />,
  ...
  return (
    <>
      <section className="w-full ml-0 m-0 h-fit px-5 pt-20 lg:pl-[320px] flex flex-col min-h-screen py-4 justify-center">
        <h1
          className={`text-[#3bb19b] text-2xl font-bold mb-2 min-[480px]:text-4xl md:text-6xl xl:text-7xl 2xl:text-8xl`}
        >
          Discover How Valuewars Operates
        </h1>
        <div className="flex flex-col gap-4 my-5">
          {steps.map((element, index) => {
            return (
              ...
              >
                <div className="bg-black text-white p-3 text-xl rounded-full w-fit group-hover:bg-[#3bb19b] transition-all duration-300">
                  {element.icon}
                </div>
                <h3
                  className={`text-[#3bb19b] text-xl font-semibold mb-2 min-[480px]:text-xl md:text-2xl lg:text-3xl`}
                >
                  {element.title}
                </h3>
